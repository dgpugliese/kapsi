...
<svelte:head>
  <title>Founders &mdash; Kappa Alpha Psi&reg; Fraternity, Inc.</title>
  <meta name="description" content="The ten founders of Kappa Alpha Psi&reg; Fraternity, Inc. — ten men who on January 5, 1911, created a legacy spanning more than a century." />
</svelte:head>
...
<section class="section">
  <div class="container">
    <h2 class="section-title">The Ten Pearls</h2>
    <div class="grid grid--2">
      {#each founders as founder}
        <div class="person-card">
          <div class="person-card-img-placeholder" aria-hidden="true">
            <span class="founder-number">{founder.number}</span>
          </div>
          <div class="person-card-body">
            <div class="person-card-name">{founder.name}</div>
            <div class="person-card-title">{founder.role}</div>
            <p class="founder-born">
              Born: {founder.born}
              {#if founder.nickname}
                <span> &mdash; &ldquo;{founder.nickname}&rdquo;</span>
              {/if}
            </p>
            <p class="founder-desc">{founder.description}</p>
          </div>
        </div>
      {/each}
    </div>
...
<section class="section section--crimson">
  <div class="container" style="text-align: center">
    <h2 class="section-title" style="color: #FFF8E1">Their Legacy Lives On</h2>
    <p style="color: rgba(255,248,225,0.85); max-width: 700px; margin: 0 auto 2rem">
    ...
    <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap">
      <a href="/history" class="btn btn--white">Read the Full History</a>
      <a href="/membership" class="btn btn--outline-white">Membership Information</a>
    </div>
  </div>
</section>
