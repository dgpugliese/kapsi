...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const [
		...
		locals.supabase.rpc('admin_status_counts'),
		locals.supabase.rpc('admin_type_counts')
	]);

	const duesCollected = (paymentsRes.data ?? []).reduce((sum, p) => sum + Number(p.amount), 0);

	// Status breakdown from RPC (no row limit)
	const statusCounts: Record<string, number> = {};
	(statusRpc.data ?? []).forEach((row: any) => { statusCounts[row.status] = Number(row.cnt); });

	// Type breakdown from RPC
	const typeCounts: Record<string, number> = {};
	(typeRpc.data ?? []).forEach((row: any) => { typeCounts[row.type] = Number(row.cnt); });
