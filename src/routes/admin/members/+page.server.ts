...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	if (hasSearch) {
		let dbQuery = locals.supabase
			.from('members')
			.select('id, first_name, last_name, email, phone, city, state, membership_status, membership_type, membership_number, role, is_staff, initiation_year, initiation_chapter, created_at', { count: 'exact' });
